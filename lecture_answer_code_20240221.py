...
if result.status != 200 : # 실패 응답
    ...
    while True : 
        ...
        if pos_open == -1 : # 못 찾으면 종료
            break
        elif html[pos_open+1] == '!' : # <! 로 시작하는 태그 무시
            start = pos_open + 1
        elif html[pos_open+1] == '/'and pos_close != -1 : # </ 로 시작하는 태그는 닫는 태그, 태그 끝도 있으면
            tag = html[pos_open+2:pos_close] # 닫는 태그 이름을 구하고
            if stack.pop() != tag : # 스택에서 여는 태그를 하나 꺼내어 비교 후 다르면 오류 처리
                success = False
                break
            start = pos_open + 1 # 다음 탐색할 위치 지정
        elif pos_close != -1 : # < 로 시작하는 여는 태그이고 태그 끝이 있으면 
            tag = html[pos_open+1:pos_close] # 여는 태그 이름을 구하고
            value = html[pos_close+1:html.find('<',pos_close+1)].strip('\n').strip('\t')  # 그 다음 태그 시작까지가 텍스트값

            # 파싱 내용을 출력
            tab = '\t'*len(stack)
            print(f'{tab}{tag} : {value}')
            
            text = text + value # 위에서 구한 텍스트값을 변수에 저장
            stack.append(tag) # 여는 태그는 스택에 넣는다
            start = pos_open + 1 # 다음 탐색할 위치 지정
